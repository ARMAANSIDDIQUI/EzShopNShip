...
import { Palette, Sun, Moon, Sparkles, Feather, Circle } from "lucide-react"
  ...
  return (
    <div className="relative">
      <Button variant="outline" size="sm" onClick={() => setIsOpen(!isOpen)} className="gap-2">
        {CurrentIcon && <CurrentIcon className={`h-4 w-4 ${currentTheme?.color}`} />}
              ...
              <div className="space-y-1">
                {themes.map((themeOption) => {
                  ...
                  return (
                    ...
                    >
                      {Icon && <Icon className={`h-4 w-4 ${themeOption.color}`} />}
